...
class MSELoss(nn.Module):
    ...
    def forward(self, x, mean, disp=None, scale_factor=None):
        # 处理scale_factor（保持与原NBLoss相同的缩放逻辑）
        # if scale_factor is not None:
        #     scale_factor = scale_factor[:, None]  # 增加维度以匹配mean的形状
        #     mean = mean * scale_factor

        # 计算均方误差（MSE）
        # mse_loss = torch.mean((x - mean) ** 2)  # 使用均值确保损失尺度稳定
        mse_loss = F.mse_loss(mean, x)
        return mse_loss
