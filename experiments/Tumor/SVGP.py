...
import numpy as np
from kernel import Cauchy_Kernel, Kernel
...
class SVGP(nn.Module):
    def __init__(self, distance,initial_inducing_points, fixed_inducing_points,fixed_gp_params, kernel_scale, jitter, N_train, dtype, device):
        ...
        if fixed_inducing_points:
            ...
            self.inducing_index_points = nn.Parameter(torch.tensor(initial_inducing_points, dtype=dtype).to(device),
                                                      requires_grad=True)
        # length scale of the kernel
        self.kernel = Kernel(distance, scale=kernel_scale, dtype=dtype, device=device).to(device)
    ...
    def kernel_matrix(self, x, y, diag_only=False):
        ...
        :return:
        """
        matrix = self.kernel(x, y, diag_only=diag_only)
        return matrix
    ...
    def variational_loss(self, x, y, noise, mu_hat, A_hat):
        ...
        K_nm = self.kernel_matrix(x, self.inducing_index_points)  # (b, m)
...
#        S = A_hat
    ...
    def approximate_posterior_params(self, x_test, x_train=None, y=None, noise=None):
        ...
        K_xm = self.kernel_matrix(x_test,self.inducing_index_points)  # (x, m)
        K_mx = torch.transpose(K_xm, 0, 1)  # (m, x)

        K_nm = self.kernel_matrix(x_train, self.inducing_index_points)  # (N, m)
    ...
    def approximate_posterior_params_big_data(self, x_test, x_train=None, y=None, noise=None):
        """
        Computes parameters of q_S.
        :param index_points_test: X_*
        :param index_points_train: X_Train
        ...
        K_xx = self.kernel_matrix(x_test, x_test).diagonal()  # (x)
        # K_xx = self.kernel_matrix(x_test, x_test, diag_only=True)  # (x)
        K_xm = self.kernel_matrix(x_test,self.inducing_index_points)  # (x, m)
        K_mx = torch.transpose(K_xm, 0, 1)  # (m, x)
